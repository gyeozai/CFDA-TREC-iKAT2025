...
class DocumentCollection:
    ...
    def write_h5(self,
        ...
        for passage_file in passage_files:
            for idx, line in enumerate(open(passage_file, encoding="utf-8")):
...
class SparseIndexer:
    ...
    def write_jsonl(self, output_dir, passage_file):
        for idx, line in enumerate(open(passage_file, encoding="utf-8")):
            ...
            with open(f"{output_dir}/data.jsonl", "a", encoding="utf-8") as f:
                f.write(json.dumps(dic) + '\n')

    @staticmethod
    def make_sparse_index(input_dir, save_path, n_threads=4):
        cmd = f"python3 -m pyserini.index -collection JsonCollection -generator DefaultLuceneDocumentGenerator -threads {n_threads} -input {input_dir} -index {save_path} -storePositions -storeDocvectors -storeRaw"      # noqa: E501
